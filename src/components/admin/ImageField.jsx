...
export default function ImageField({ id, value, onChange, multiple = false, setUploading, maxImages = 8 }) {
  ...
  async function upload(event) {
    ...
    if (multiple && images.length + selected.length > maxImages) {
      setError(`Use up to ${maxImages} photos for this record.`); return;
    }
    ...
    try {
      ...
      setError((error.message || "The upload could not finish.") + " Successfully uploaded photos remain below.");
    } finally {
    ...
    <div className="admin-image-grid">{images.map((raw, index) => {
      ...
      return <div className="admin-image" key={`${image.publicId || image.url || "photo"}-${index}`}>
        ...
        <div className="admin-actions">
          {multiple && index > 0 && <Button variant="ghost" onClick={() => onChange([images[index], ...images.filter((_, i) => i !== index)])}>Make cover</Button>}
          {multiple && index > 0 && <Button variant="ghost" onClick={() => move(index, -1)} aria-label={`Move photo ${index + 1} earlier`}>Move earlier</Button>}
          {multiple && index < images.length - 1 && <Button variant="ghost" onClick={() => move(index, 1)} aria-label={`Move photo ${index + 1} later`}>Move later</Button>}
          <Button variant="ghost" onClick={() => onChange(multiple ? images.filter((_, i) => i !== index) : null)}>Remove photo {index + 1}</Button>
        </div>
      </div>;
    })}</div>
    {multiple && <p className="field__hint">{images.length} of {maxImages} photos uploaded. Photo 1 is the primary image shown on the homepage.</p>}
